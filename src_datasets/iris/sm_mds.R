...
# 1) Sammon Mapping
d0 <- dist(iris_j) # d0 se sigue necesitando para las métricas

sm <- Rdimtools::do.sammon(iris_j, ndim = 2)
emb <- sm$Y

# 2) Métricas
d1 <- dist(emb)
cat(sprintf("Spearman=%.3f\n", cor(as.vector(d0), as.vector(d1), method = "spearman")),
    file = "results/metrics_config_sammon.txt") # Nombre de archivo original
...
p <- ggplot(sam_df, aes(Dim1, Dim2, color = Species)) +
  geom_point(size = 2) +
  ggtitle("Sammon Mapping") # Título original del gráfico

# Asumimos que plot_theme.R o el entorno se encargan de ggplot2 y big_text_theme si es necesario
# como en tu script original.

print(p)
ggsave("plots/Sammon_Mapping_plot.png", plot = p, dpi = 300, width = 8, height = 6) # Nombre de archivo original

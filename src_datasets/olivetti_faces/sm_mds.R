...
cat(sprintf("Spearman=%.3f\n", spearman), file = "results/metrics_config_sammon_olivetti.txt")
cat(sprintf("ARI=%.3f\n", ari), file = "results/metrics_config_sammon_olivetti.txt", append = TRUE)
cat(sprintf("Silhouette=%.3f\n", sil), file = "results/metrics_config_sammon_olivetti.txt", append = TRUE)
cat(sprintf("Trustworthiness=%.3f\n", trust), file = "results/metrics_config_sammon_olivetti.txt", append = TRUE)
cat(sprintf("Continuity=%.3f\n", cont), file = "results/metrics_config_sammon_olivetti.txt", append = TRUE)

# Plot
sam_df <- data.frame(Dim1 = emb[,1], Dim2 = emb[,2], Class = labels)
p <- ggplot(sam_df, aes(Dim1, Dim2, color = Class)) +
  geom_point(size = 1) +
...
ggsave("plots/Sammon_Mapping_olivetti_plot.png", plot = p, width = 8, height = 6, dpi = 300)
